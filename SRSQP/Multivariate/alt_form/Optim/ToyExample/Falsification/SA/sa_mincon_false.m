...
function [bestsol,fmin,N,info]=sa_mincon_false(alpha,optParams,x0)
ExactParams = [];
...
Lb = -optParams.P_init.b(1:Nx);
Ub = optParams.P_init.b(Nx+1:end);
...
max_run=200;       % Maximum number of runs, 500 def
max_accept = 1000;   % Maximum number of accept, 250 def
initial_search=150; % Initial search period 
...
x0 = u;
z = objfun_toy_falsification(x0,optParams,1);

% All constraints
function [g,geq]=constraints(u,optParams)
geq = 0;
x0 = u;
g = optParams.P_init.A*x0-optParams.P_init.b;
%% End of the program ------------------------------------------------
